...
function KitchenCleaningForm({ serviceTitle, slug }) {
  ...
  useEffect(() => {
    if (user?.email) {
      ...
      if (userBookings.length > 0) {
        ...
        console.log("KitchenCleaningForm: Pre-filled date/time from latest booking:", {
          date: selectedDate,
          time: selectedTime,
        });
      }
  ...
  const calculateTotalCost = () => {
    const baseCost = 80;
    const kitchenTypeMultiplier = formData.kitchenType === "Commercial" ? 1.5 : 1;
    const applianceCost = formData.appliances.length * 10;
    return (baseCost * kitchenTypeMultiplier + applianceCost).toFixed(2);
  };
  ...
  const validateStep1 = () => {
    ...
    if (!formData.kitchenType) newErrors.kitchenType = "Kitchen type is required";
    if (formData.appliances.length === 0) newErrors.appliances = "At least one appliance must be selected";
    setErrors(newErrors);
  ...
  const handleSubmit = async () => {
    if (validateStep2()) {
      ...
      try {
        ...
        console.log("KitchenCleaningForm: Saving to localStorage:", bookingData);
    ...
    <div className="booking-container">
      ...
      {step === 1 && (
        <div className="form-section">
          ...
          <h3 className="form-heading">Kitchen Details</h3>
          <div className="form-group">
            <label htmlFor="kitchenType">Kitchen Type</label>
            <select id="kitchenType" name="kitchenType" value={formData.kitchenType} onChange={handleChange}>
              <option value="">Select kitchen type</option>
              <option value="Residential">Residential</option>
              <option value="Commercial">Commercial</option>
            </select>
            {errors.kitchenType && <p className="error-message">{errors.kitchenType}</p>}
          </div>
          <div className="form-group">
            <label>Appliances to Clean</label>
            <div className="checkbox-group">
              {["Oven", "Fridge", "Microwave", "Dishwasher"].map((appliance) => (
                <label key={appliance}>
                  <input
                    type="checkbox"
                    name="appliances"
            ...
            {errors.appliances && <p className="error-message">{errors.appliances}</p>}
          </div>
